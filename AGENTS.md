...
| Kernel package input | [`archiso/pacman.conf`](archiso/pacman.conf), [`local-repo/`](local-repo) | The committed pacman file currently has a developer-local `file://` path that must be replaced per build. |
...
- Do not assume the committed `[macpro]` repo URL is portable. It is currently a local developer path.
...
- If CI changes are touched, validate that the workflow can provide the `[macpro]` package repository before expecting ISO builds to pass.
- Heavy ISO builds, live USB boot tests, and Mac Pro hardware tests are expensive. If they were not run, say so explicitly.

## Watchpoints

- [`archiso/pacman.conf`](archiso/pacman.conf) currently points `[macpro]` at `/home/michael/.../local-repo`; this is not portable.
